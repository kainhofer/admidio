...
class SSOKeyPresenter extends PagePresenter
{
    ...
    public function createEditForm(string $mode = "view"): void
    {
        ...
        // $form->addSelectBox(
        //     'key_type',
        //     $gL10n->get('SYS_SSO_KEY_TYPE'),
        //     ['saml_signing' => "SAML 2.0 signing", 'saml_encryption' => 'SAML 2.0 encryption', 'oidc_signing' => 'OIDC signing', 'oidc_encryption' => 'OIDC encryption'],
        //     array('defaultValue' => $key->getValue('key_type')?:'oidc_signing', 'property' => FormPresenter::FIELD_REQUIRED, 'helpTextId' => $gL10n->get('SYS_SSO_KEY_TYPE_DESC'))
        // );
        ...
        // The certificate expiration will be stored in the database, but we can't directly use the 
        // key_expires_at field because it is a datetime field and the html form uses two input controls
        // for date and time -> use 'cert_expiration' as a temporary name and plug 'cert_expiration' 
        // and 'cert_expiration_time' together in the php handler when saving data from the form.
        $form->addInput(
            'key_expires_at',
        ...
        if ($mode == 'edit' && $haveKey) {
//            $form->addHiddenField('key_uuid', $this->keyUUID);
        } elseif ($mode == 'edit' && !$haveKey) {

        }

        
        // Add a "Edit certificate data" button that makes all certificate data fields editable.
        // NB: The certificate with modified data needs to be generated by a click on "Save" or "Re-Generate Certificate"
        $form->addButton(
            'adm_button_certificate_dataedit', 
            $gL10n->get('SYS_SSO_CERTIFICATE_EDIT'), 
            array('icon' => 'bi-pencil-square', 
                'class' => 'offset-sm-0 btn-secondary ', 'style' => 'margin-bottom: 0pt'));
        $this->addJavascript("
            $(\"#adm_button_certificate_dataedit\").on(\"click\", function (event) {
                event.preventDefault();
                $(\".certdata\").prop(\"disabled\", false);
            });", true);

        $form->addSubmitButton(
            'adm_button_save', 
            $gL10n->get('SYS_SAVE'), 
            array('icon' => 'bi-check-lg', 'class' => 'offset-sm-3', 'name' => 'adm_button', 'btn-value' => 'save'));
        $form->addSubmitButton(
            'adm_button_certificate', 
            $gL10n->get('SYS_SSO_NEW_CERTIFICATE'), 
            array('icon' => 'bi-shield-lock', 'class' => 'offset-sm-0', 'name' => 'adm_button', 'btn-value' => 'certificate'));
        $form->addSubmitButton(
            'adm_button_regenerate', 
            $gL10n->get('SYS_SSO_KEY_REGENERATE'), 
            array('icon' => 'bi-key', 'class' => 'offset-sm-0', 'name' => 'adm_button', 'btn-value' => 'key'));

        /*******************************************
         * TODO: Button to re-generate a new key
         */
        $form->addButton('adm_button_recreate_key', $gL10n->get('SYS_SSO_KEY_RECREATE'), array('icon' => 'bi-gear-fill', 'class' => 'btn btn-primary'));
    ...
    public function createList(): void
    {
        ...
        // $this->addHtml('<p>' . $gL10n->get('SYS_SSO_KEY_ADMIN_DESC') . '</p>');
        ...
        foreach ($keyService->getKeysData() as $keyData) {
            ...
            $actions = array(
                ...
                // array('mode' => 'recreate',    'icon' => 'bi-arrow-repeat',   'title' => $gL10n->get('SYS_SSO_KEY_REGENERATE')),
